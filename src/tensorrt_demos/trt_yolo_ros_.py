...
cls_names = {0:"balls", 1:"circle_dock", 2:"port_buoy", 3:"square_dock", 4:"starboard_buoy",5:"triangle_dock", 6:"water_target"}
...
#              '288, 416, 608) or 2 numbers, WxH (e.g. 416x256)'))
    parser.add_argument(
           '-m', '--model', type=str, default = 'yolov4_korban'
                 )
...
def infer_service_cb(req : StringServiceRequest):
    ...
    return ret
conf_th = 0.5
def conf_service_cb(req : FloatServiceRequest):
    global conf_th
    conf_th = np.clip(req.data,0,1)
    ret = FloatServiceResponse()
    ret.status = "conf_th changed to " + str(conf_th)
    return ret

inferService = rospy.Service('/vision/infer',StringService,infer_service_cb)
confService = rospy.Service('/vision/conf',FloatService,conf_service_cb)
...
obstacleline = 0.0
def cb_obstacleline(msg : Float32):
    global obstacleline
    obstacleline = msg.data
sub_crit = rospy.Subscriber('/debug_critline',Float32,cb_critline,queue_size=1)
sub_obst = rospy.Subscriber('/debug_obstacleline', Float32, cb_obstacleline, queue_size=1)
...
def loop_and_detect(cam, trt_yolo, conf_th_, vis, args):
    global bridge, imgPublisher, objPublisher, infer, brightness, gamma_table
    global boatsidex, boatsidey, critline, controlpoint
    global conf_th
    ...
    while not rospy.is_shutdown():
        ...
        if infer:
            ...
            if args.clahe:
                img_lab_l, img_lab_a, img_lab_b = cv2.split(cv2.cvtColor(img,cv2.COLOR_BGR2LAB))
                clahed_l = clahe.apply(img_lab_l)
                clahed_lab = cv2.merge([clahed_l, img_lab_a, img_lab_b])
                img = cv2.cvtColor(clahed_lab,cv2.COLOR_LAB2BGR)
            ...
            for i,bb in reversed(iob):
                ...
                color_idx = 4
                bbx_color = [(0,0,255),(0,255,255),(0,255,0),(255,0,0),(0,0,0)]
                ...
                if cls_name == "balls":
                    ...
                    sats = img_hsv[y_min:y_max,x_min:x_max,1]
                    ...
                    if avg_hue>110 or avg_hue < 15:
                        cls_name="red_ball"
                        color_idx = 0
                    elif avg_hue>=15 and avg_hue < 35:
                        cls_name="yellow_ball"
                        color_idx = 1
                    elif avg_hue>=35 and avg_hue < 100:
                        cls_name="green_ball"
                        color_idx = 2
                    elif avg_hue>=100 and avg_hue <110:
                        cls_name="blue_ball"
                        color_idx = 3
                    pass
                if cls_name == "balls" and False:
                ...
                cv2.rectangle(img_to_send,(x_min,y_min),(x_max,y_max),bbx_color[color_idx],2)
        ...
        imgMsg = bridge.cv2_to_compressed_imgmsg(img, dst_format = 'jpg')
        ...
        cv2.line(img_to_send,(int(0),int(obstacleline*480)),(640,int(obstacleline*480)),(0,255,255),3)
        # controlpoint
        cv2.line(img_to_send,(int(controlpoint[1]),int(controlpoint[0])),(int(controlpoint[3]),int(controlpoint[2]) ),(0,0,255),4)

        encoded, send_buffer = cv2.imencode('.jpg',img_to_send,[int(cv2.IMWRITE_JPEG_QUALITY), 20])
        try:
...
def main():
    ...
    loop_and_detect(cam, trt_yolo, conf_th_=0.7, vis=vis, args = args)
